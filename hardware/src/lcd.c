...
{ 	
  ...
  FSMC_NORSRAMCmd(FSMC_Bank1_NORSRAM4, ENABLE);  // 使能BANK1 
	...
	}else if(lcddev.id==0X9331)//OK |/|/|			 
	{
		...
		vTaskDelay(200); // Dis-charge capacitor power voltage
...
//color:颜色 
//num:数值(0~4294967295);	
void LCD_ShowDJ(uint16_t x,uint16_t y,char Hzk[][16],uint8_t chr,uint8_t mode)
{  							  
  uint8_t temp,t1,t;
	uint16_t y0=y;
	uint8_t size = 16;
...
void LCD_ShowCHinese(uint8_t x,uint8_t y,char Hzk[][16],uint8_t no)
{      			    
	uint8_t i=0;
	for(;i<no*2;i++)
		LCD_ShowDJ(x+8*i,y,Hzk,i,1);
}
